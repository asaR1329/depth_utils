...
def convert_pcd(fname):
    ...
    ### PCDファイル作成
    if points_num!=0 and int(fname)%10==0: #点が存在 and 0.1秒刻み
        ...
        print(f'---end convert pcd {fname} {points_num}---')
### ->pcd file
...
def convert_pcds(fname):
    ...
    for ln in range(int(fname)):
        ...
        #なければ作成
        try:
            with open(cfname, mode='x') as fp:
                # print('convert_txt2pcd:make new pcd file')
                myfile = Path(f'output/{cfname}.pcd')
