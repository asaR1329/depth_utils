...
def img2world(moms):
    ### image2cam
    calib_path = './calibration/cam_to_cam.yaml'
    ...
    ### cam2world
    calib_path = './calibration/cam_to_lidar.yaml'
            ...
            for j in range(len(tracer[i])):     # i   の全重心
                ...
                for k in range(len(tracer_wID[i-1])):   # i-1 の全重心
                    try:
                        ...
                        print(f' i={i} j(a)={j} k(b)={k} a={a} b={b} dist={distance}')
...
def make_tracer(fnumber, tm_):
    ...
    try:
        ...
        for ll in range(tm_):
            ...
            gfname  = f'input/{fname}_{f1number}.txt'
            ssfname = f'dataset/{f2number}.png'
    ...
    for i in range(len(tracer_wID)):
        for j in range(len(tracer_wID[i])):
            print(f'frame ={i:3d} ID ={tracer_wID[i][j][0]:2.0f} x = {tracer_wID[i][j][1]:9.6f} y = {tracer_wID[i][j][2]:9.6f} depth = {tracer_wID[i][j][3]:.4g}')
    ###
